...
export function RadioButtonGroup(props: any) {
  ...
  const cartData = () => {
    ...
    if (convId && botType && convId !== "" && botType !== "") {
      pageData(newData, botType).then((data) => {
        console.log("raju", data.data.activities[0].value.data);
        if (data && data.data.activities[0].type === "address") {
          setAddressArray(data.data.activities[0].value.data);
        }
      });
    }
  };
        ...
        <>
          <label className="flex items-start mt-2 mb-3 px-5" key={index}>
            <input
              ...
              className="mt-4"
              value={index}
              checked={props?.selectedOption && props?.selectedOption == index}
            ...
            <div className="ms-2 w-full">
              ...
              >
                <img src="/images/edit.svg" height="12px" width="12px"></img>
              </div>
              <div className="text-black text-sm font-medium">{item.Name}</div>
